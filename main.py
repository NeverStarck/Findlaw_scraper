...
def create_directory(parent_directory, child_directory=None):
    ...
    if os.path.exists(custom_directory):
        print(f"The directory '{child_directory}' already exists inside '{parent_directory}'.\n")
    else:
        os.makedirs(custom_directory)
        print(f"Directory '{child_directory}' created successfully inside '{parent_directory}'.\n")

def get_all_states() -> list[str]:
    req = scraper.get(f"https://lawyers.findlaw.com/")
    if req.status_code != 200:
        print(f"Failed to retrieve page: States")
        return []
...
def get_all_state_cities(state) -> list[str]:
    ...
    if req.status_code != 200:
        print(f"Failed to retrieve page: State Cities")
        return []
...
def get_all_practices() -> list[str]:
    ...
    if req.status_code != 200:
        print(f"Failed to retrieve page: Practices")
        return []
...
def get_profile_data(url) -> dict[str, any]:
    req = scraper.get(url)
    if req.status_code != 200:
        print(f"Failed to retrieve page: Profile - {url}")
        return {}
...
for state in states:
    ...
    for practice in practices:
        print(f"Getting PRACTICE AREA: {practice}\n")

        create_directory(state, practice)

        for city in cities:
            print(f"Getting CITY: {city}\n")

            # check if was already scraped
            ...
            # profiles_url = ['https://lawyers.findlaw.com/profile/view/5087180_1', 'https://lawyers.findlaw.com/profile/view/5087180_1']
            profiles_data = []
